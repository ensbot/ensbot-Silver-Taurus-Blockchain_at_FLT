...
#     it has to be below the target and the reason to do that is just to create hurdles for minor to found the hash-number. So when the hash
#     is below the target then you are consider as the minor who mines the block. Since the block-number, data and previous-hash cannot be
#     changed, the only factor used to change the hash values is nonce. The minor has to find a nonce for which the hash is below the target
...
#   This is nothing but the concept, that the minor has done the work in finding the golden nonce and hence mining the block in the chain.
#   The proof is the acceptabe hash.
#
#   - Solutions for the challenges:
#       - For this mining, the minor gots the reward which lets the minor play fair, because if they add a malicious block then, they will not 
...
#
# So, the Consensus Protocol in Blockchain is much more powerfull than of in Byzantine Problem.
